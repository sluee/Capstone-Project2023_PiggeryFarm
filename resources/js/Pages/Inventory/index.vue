    ...
    <SideBarLayout>
        <template #header >
            <div class="flex justify-between">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Feeds Inventory</h2>
                ...
                <div class="flex">
                    <Link class="button1 mb-2 py-2 px-3 bg-blue-300 shadow border-gray-300 border hover:bg-blue-400 rounded mr-3" as="button" href="/inventories/create">Add Item</Link>
                    ...
                    <table class="min-w-max w-full table-auto">
                        <thead>
                            <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                                ...
                                <th class="py-3 px-6 text-left">Unit Discount</th>
                        ...
                        <tbody class="text-gray-600 text-sm font-light" >
                            ...
                            <tr  class="border-b border-gray-200 hover:bg-gray-100" v-for="inv in inventories" :key="inv.id">
                                ...
                                <td class="py-3 px-6 text-left">
                                    <div class="flex items-center">
                                        <p class="font-medium">{{ inv.supplier.name }}</p>
                                    </div>
                                </td>
                                <td class="py-3 px-6 text-left">
                                    <div class="flex items-center">
                                        <p class="font-medium">{{ inv.category.name }}</p>
                                    </div>
                                </td>
                                <td class="py-3 px-6 text-left">
                                    <div class="flex items-center">
                                        <p class="font-medium">{{ inv.feed.name }}</p>
                                    </div>
                                </td>
                                <td class="py-3 px-6 text-center">
                                    <div class="flex items-center justify-center">
                                        <p class="font-medium">{{ inv.qty}}</p>
                                    </div>
                                </td>
                                <td class="py-3 px-6 text-center">
                                    <span class="rounded-full text-sm">{{ formattedDate(inv.date_received) }}</span>
                                </td>
                                <td class="py-3 px-6 text-left">
                                    <div class="flex items-center justify-center">
                                        <p class="font-medium">{{ inv.costPerStocks }}</p>
                                    </div>
                                </td>
                                <td class="py-3 px-6 text-left">
                                    <div class="flex items-center justify-center">
                                        <p class="font-medium">{{ inv.discount }}</p>
                                    </div>
                                </td>
                                <td class="py-3 px-6 text-left">
                                    <div class="flex items-center justify-center">
                                        <p class="font-medium">{{ inv.totalAmountAfterDiscount }}</p>
                                    </div>
                                </td>
                                ...
                                <td class="py-3 px-6 text-center">
                                    <div class="flex item-center justify-center">
                                        ...
                                        <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                            <button class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110" @click="updateItem(inv.id)">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                            <button class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110" @click="deleteItem(inv.id)">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function search(ev){
    router.visit('/inventories/search/'+ ev.target.value);
}

function updateItem(itemId) {
    router.visit('/inventories/edit/' + itemId);
}

function deleteItem(itemId) {
  if (confirm('Are you sure you want to remove selected item?')) {
    try {
      Inertia.delete(`/inventories/${itemId}`)
